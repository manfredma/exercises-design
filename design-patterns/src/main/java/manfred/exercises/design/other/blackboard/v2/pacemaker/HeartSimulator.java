...
/**
 * 心脏模拟器，对心脏发送脉冲信号进行简单模拟，偶尔模拟漏跳情况。
 * 作为演示入口，可通过 main 方法独立启动，与起搏器传感器配合使用。
 */
public class HeartSimulator {
    // The time interval between heart pulses in milliseconds. An interval of
    // 1000 milliseconds equates to a heart rate of 60 beats per minute.
    ...
    private static final double heartRateDelta = 0.15;
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try (Socket pacemakerSocket = new Socket(hostName, portNumber);
                ...
                int newPulseInterval =
                        randomNumber.nextInt((maxPulseInterval - minPulseInterval) + 1)
                                + minPulseInterval;
                ...
                System.err.println(
                        "Couldn't get I/O for the connection to " + hostName + ". Retrying...");
            }
        } // End While
    }
}
