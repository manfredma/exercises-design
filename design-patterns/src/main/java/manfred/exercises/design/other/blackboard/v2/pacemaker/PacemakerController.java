...
import manfred.exercises.design.other.blackboard.v2.heartbeat.HeartbeatReceiver;

import java.util.ArrayList;

/**
 * 起搏器控制器，心脏起搏器应用的主控类（演示入口）。
 * 负责显示 GUI 并配置心跳接收器、传感器等系统组件，实现传感器故障自动切换。
 */
public class PacemakerController {
    // Specify how often we want to check for the aliveness of the heartbeat
    // sender
    private static final long checkingInterval = 1000;
    ...
    // plus a small buffer to account for possible delays
    ...
    private static final ArrayList<PacemakerSensor> sensorsList = new ArrayList<PacemakerSensor>();
    ...
    private static void startNewSensor() {
        for (PacemakerSensor sensor : sensorsList) {
            if (!sensor.getSensorName().equals(brokenSensor.getSensorName())) {
                activeSensor = sensor;
                activeSensor.start();
    ...
    private static void reconfigureBrokenSensor() {
        for (PacemakerSensor sensor : sensorsList) {
            if (sensor.getSensorName().equals(brokenSensor.getSensorName())) {
                ...
                System.err.println(
                        "Failed sensor (" + brokenSensor.getSensorName() + ") was removed...");
                break;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        Thread heartbeatReceiver = new Thread(new HeartbeatReceiver(checkingInterval, expireTime));
