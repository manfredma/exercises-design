...
package manfred.exercises.design.arch.dci.context;
...
/**
 * DCI 转账上下文（Polygene Mixin 版本），封装两个账户之间转账的角色与交互逻辑。
 * 角色定义在上下文内部，只有 RoleMap 在上下文之外感知它们。
 */
public class TransferMoneyContext {
    ...
    public void transfer(int amount) throws IllegalArgumentException {
    ...
    public interface SourceAccountRole {
        // Role Methods
        void transfer(int amount, DestinationAccountRole destinationAccount)
    ...
    public interface DestinationAccountRole {
        ...
        class Mixin implements DestinationAccountRole {
            @This
            BalanceData data;
