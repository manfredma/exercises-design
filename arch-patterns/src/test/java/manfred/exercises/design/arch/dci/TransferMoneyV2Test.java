...
package manfred.exercises.design.arch.dci;

import static org.apache.polygene.api.usecase.UsecaseBuilder.newUsecase;

import manfred.exercises.design.arch.dci.context.PayBillsContext2;
import manfred.exercises.design.arch.dci.context.TransferMoneyContext2;
import manfred.exercises.design.arch.dci.domain.data.BalanceData;
import manfred.exercises.design.arch.dci.domain.entity.CheckingAccountEntity;
import manfred.exercises.design.arch.dci.domain.entity.CreditorEntity;
import manfred.exercises.design.arch.dci.domain.entity.SavingsAccountEntity;
...
/**
 * TransferMoneyContext2 转账上下文测试（Methodful Role 版本）。
 */
public class TransferMoneyV2Test implements AccountIds {
    ...
    public static void setup() throws Exception {
        SingletonAssembler assembler = new SingletonAssembler(
                moduleAssembly -> {
                    moduleAssembly.entities(CheckingAccountEntity.class, SavingsAccountEntity.class,
    ...
    private static void bootstrapData() {
        ...
        try {
            SavingsAccountEntity account = uow.newEntity(SavingsAccountEntity.class,
                    SAVINGS_ACCOUNT_ID);
            account.increasedBalance(1000);

            CheckingAccountEntity checkingAccount = uow.newEntity(CheckingAccountEntity.class,
                    CHECKING_ACCOUNT_ID);
